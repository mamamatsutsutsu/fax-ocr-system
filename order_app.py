...
def run():
    ...
    def create_pdf(df):
        ...
        font_name = "HeiseiKakuGo-W5" # デフォルト（日本語非対応環境用）
        try:
            # カレントディレクトリにフォントファイルがあるか確認
            if os.path.exists("ipaexg.ttf"):
                pdfmetrics.registerFont(TTFont('IPAexGothic', 'ipaexg.ttf'))
                font_name = 'IPAexGothic'
            else:
                st.warning("⚠️ 日本語フォント(ipaexg.ttf)が見つかりません。PDFが文字化けする可能性があります。")
        except:
            pass
        ...
        for index, row in df.iterrows():
            ...
            customer = str(row.get('顧客名', ''))[:10] # 長すぎると重なるのでカット
        ...
        for i, file in enumerate(uploaded_files):
            ...
            try:
                ...
                if file.type == "application/pdf":
                    # PDF対応（本来はpdf2imageが必要だが、簡易的にエラー回避）
